...
// Criando arquivo User.js
$fp = fopen('./js/user.js', 'w+');
...
<html>
    <head>
        ...
        <script src="js/jquery.maskInput.js" type="text/javascript"></script>
        ...
        <div data-role="page" id="home">
            ...
            <div data-role="content">
                ...
                <div data-role="collapsible-set" style="width:100%">
                    <div data-role="collapsible" style="width:100%" data-collapsed="true">
                ...
                <p><?= $mob->homeSaldoContas() ?></p>
                <p><?= "Saldo passivos: R$" . $saldoPassivos['saldoFormatado'] ?></p>
                <div id="divBalancoGeral"></div>
            </div>
            <!-- close content home -->
        ...
        <div data-role="page" id="config">
            ...
            <div data-role="content">
                <ul data-role="listview" data-split-icon="forward">
                    <? if ($_SESSION['idUser'] == 1)   { // Cristofer ?>
                    <li>
...
<p><?= $mob->getPizza() ?></p>
        ...
        <div data-role="page" id="finalizarRegistro">
            ...
            <div data-role="content">
                ...
                <p>
                    <b>Parcela: </b><span id="spParcela"></span>
                </p>
            ...
            <div data-role="content">
                <div data-role="fieldcontain">
                    ...
                    <fieldset data-role="controlgroup" data-type="horizontal">
                        <legend>Opção: </legend>
                        <input type="radio" name="opcao" id="opcao_0" class="btnOpcao" value="+" /> 
                        <label for="opcao_0">Entrada</label> 

                        <input type="radio" name="opcao" id="opcao_1" class="btnOpcao" value="-" />
                        <label for="opcao_1">Saida</label>

                        <input type="radio" name="opcao" id="opcao_2" class="btnOpcao" value="TR" />
                        <label for="opcao_2">Transferência entre contas</label>
                    </fieldset>
                    ...
                    <p id="addRegistro_Parcelas" style="display:none"></p>
        ...
        <div data-role="page" id="addTef">
            <?= Analizze::geraHeader(array("texto" => "Voltar", 'link' => 'javascript:history.back()', 'icone' => 'arrow-l'), "Finalizar Transferência", array('texto' => 'Concluir', 'link' => 'javascript: void(0)', 'icone' => 'check', 'class' => 'btnConcluirTef')) ?>
            <div data-role="content" id="addTefContent">
                <div data-role="fieldcontain">
                    <label for="tefValor">Valor a transferir:</label>
                    <input type="text" id="tefValor" value="" placeholder="Valor"  />
                </div>

                <div data-role="fieldcontain">
                    <label for="selectmenu" class="select"><b>Conta Origem </b> </label>
                    <select id="tefContaDebito"><?= $mob->getRelacaoContas() ?>
                    </select>

                </div>

                <div data-role="fieldcontain">
                    <label for="selectmenu2" class="select"><b>Conta Destino: </b> </label>
                    <select id="tefContaCredito"><?= $mob->getRelacaoContas() ?>
                    </select>
                    ...
                    <div id="reocorrencia_escolhe" class="reocorrencia">
                        <fieldset data-role="controlgroup" data-type="horizontal">
                            ...
                            <div id="qtdeOcorrencias" style="display: none">
                                <label for="textinput">Quantidade meses (incluindo este):</label> <input
        ...
        <div data-role="page" id="config_data">
            ...
            <div data-role="content">
                ...
                <p>Os lançamentos serão exibidos na relação e no balanço até o dia anterior ao dia escolhido como inicio do ciclo. </p>
                ...
                <div data-role="fieldcontain">
                    ...
                    <input type="text" name="diaCorteText" id="diaCorteText" value="<?= $_SESSION['diaInicioContabil'] ?>" style="width:50px;" />
                    <br /><br />
                    <label for="textinput">Qtde Meses a exibir: (Utilize "0" para padrão ou a quantidade de meses além do padrão)</label>
                    <input type="text" name="mesesFuturos" id="mesesFuturos" value="<?= $_SESSION['mesesFuturos'] ?>" style="width:50px;" />
                ...
                <script>
                    ...
                    function setRegistro(id, loc, ret) {
                        if (id != "") {
                            ...
                            if (ret['parcelas'] != '') {
                                $("#addRegistro_Parcelas").html('Lancamento: ' + ret['parcelas']).fadeIn();
                            }
                            if (ret['valor'] > 0) {
                            ...
                            $("#spParcela").html(ret['parcelas']);
                            $("#spVencimento").html(ret['vencimento']);
                            if (ret['valor'] > 0) {
                                $("#spValor").removeClass().addClass("positivo");
                                $("#spTipo").html("Entrada");
                            }
                            else {
                                $("#spValor").removeClass().addClass("negativo");
                                $("#spTipo").html("Saída");
                            }
                            ///******************************/
                    ...
                    function extratoShow(id)   {
                        var id = id.replace("homeSaldoConta_", "");
                        $("#extratoShow").html("Obtendo extrato...");
                        aut.getextratocc(id, function(ret)   {
                            $("#extratoShow").html(ret);
                        });
                    }
                    ...
                    $(document).ready(function () {
                        ...
                        var forb = Array("#addRegistro", "#addRegistroStep2", "#finalizarRegistro", "#addTef"); // se hash for uma dessas, transferir para home, com recarga
                        ...
                        /* retirei pois os navegadores foram ajujstados ao jquery mask
                         if (agentID) { // mobile
                         $("#vencimento").keyup(function() {
                         var valAtual = $("#vencimento").val();
                         if (valAtual.length == 2)
                         $("#vencimento").attr('value', valAtual + "/");
                         if (valAtual.length == 5)
                         $("#vencimento").attr('value', valAtual + "/");
                         });
                         $("#relDataInicio").keyup(function() {
                         var valAtual = $("#relDataInicio").val();
                         if (valAtual.length == 2)
                         $("#relDataInicio").attr('value', valAtual + "/");
                         if (valAtual.length == 5)
                         $("#relDataInicio").attr('value', valAtual + "/");
                         });
                         ...
                         if (valAtual.length == 2)
                         $("#relDataFim").attr('value', valAtual + "/");
                         if (valAtual.length == 5)
                         $("#relDataFim").attr('value', valAtual + "/");
                         });
                         ...
                         */
                        $("#valor").maskMoney({showSymbol: true, prefix: "R$ ", decimal: ",", thousands: "."});
                        $("#tefValor").maskMoney({showSymbol: true, prefix: "R$ ", decimal: ",", thousands: "."});
                        $("#vencimento").mask("99/99/9999");
                        $("#relDataInicio").mask("99/99/9999");
                        $("#relDataFim").mask("99/99/9999");
                        ...
                        $(".removeRegistro").click(function () {
                            ...
                            var confimar = '';
                            if (qualExcluir == 1)
                                confirmar = "Confirma a exclusão deste registro?";
                            else if (qualExcluir == 2)
                                confirmar = "Confirma a exclusão deste e dos futuros lançamentos?";
                            else
                                confirmar = "Confirma a exclusão de todas as ocorrências? \n\nATENÇÃO: Este processo é irreversível!";
                        ...
                        $(".avancaRegistro").click(function () {
                            ...
                            if (erro == false) {
                                if (cat == "TR") {
                                    $("#tefValor").attr('value', $("#valor").val());
                                    window.location = "#addTef";
                                }
                                else {
                        ...
                        /**
                         * Concluir execução de TEF
                         */
                        $(".btnConcluirTef").click(function () {
                            var ctaDebito = $("#tefContaDebito").select().attr('value');
                            var ctaCredito = $("#tefContaCredito").select().val()
                            var valor = $("#tefValor").attr('value');
                            var contextoAtual = $("#addTefContent").html();
                            $("#addTefContent").html('<br /><br /><?= $mob->getLoading() ?> Executando Registro ...').fadeIn();

                            aut.transferenciaentrecontas(ctaDebito, ctaCredito, valor, function (ret) {
                                if (ret === true) {
                                    alert("Transferência efetuada com sucesso");
                                    window.location = "index.php";
                                }
                                else if (ret === false) {
                                    alert("Infelizmente, ocorreu uma falha na transferência. \n\nPode deixar que vou repassar a informação para arrumarem isso. :(")
                                    window.location = "index.php";
                                }
                                else {
                                    $("#addTefContent").html(contextoAtual);
                                    alert(ret);
                        ...
                        $(".btnConcluir").click(function () {
                            ...
                            aut.addregistro(dados, idRegistro, function (ret) {
                                if (ret) {
                                    ...
                                    alert("Ocorreu um erro inesperado. O Administrador já foi avisado, mas os registros não foram inseridos.");
                                    window.location = "index.php"
                                }
                            });
                        });
                        ...
                        $(".btAlteraDiaCorte").click(function () {
                            if ($("#diaCorteText").val() != "") {
                                ...
                                dados['mesesFuturos'] = $("#mesesFuturos").val();
