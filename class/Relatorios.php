...
class Relatorios extends Analizze  {
	var $con, $header, $status, $statusLabel, $dataInicio, $dataFim, $tipo, $nomeRelatorio, $idCat;
	...
	public function getLancamentos()   {
		$idCat = false;
		$analizze = new Analizze();
		$con = new Conexao();
		switch ($this->tipo)   {
			case 1: // Contas a Receber
				$query[] = " valor > 0";
				if ($_SESSION['idCatCredito']!='')  $idCat = $_SESSION['idCatCredito'];
				$this->setNomeRelatorio("Contas a Receber");
				$txt = "Recebimento ";
				break;
			case 2: // Contas a Pagar
				$query[] = " valor < 0";
				if ($_SESSION['idCatDebito']!='')  $idCat = $_SESSION['idCatDebito'];
		...
		if ($idCat)   {
			$temp = $analizze->getDados('anz_categorias', 'idCat', $idCat);
			$query[] = 'l.idCat = ' . $idCat;
			$this->setStatusLabel('Categoria: ' . utf8_decode($temp['label']) . ' - '. $this->getStatusLabel());
		}
			

		if ($this->getIdCat()!='') $query[] = 'l.idCat = ' . $this->getIdCat();
